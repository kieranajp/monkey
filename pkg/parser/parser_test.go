...
func TestIdentifierExpression(t *testing.T) {
	...
	if !ok {
		t.Fatalf("Unexpected statement, got=%d, expected=foobar;", program.Statements[0])
	}

	ident, ok := stmt.Expression.(*ast.Identifier)
	if !ok {
		t.Fatalf("Unexpected statement, got=%s, expected=foobar;", program.Statements[0])
	}
	if ident.Value != "foobar" {
		t.Errorf("Unexpected ident.Value, got=%s, expected=foobad", ident.Value)
	}
